...
# Mount static files
# /static/* — audio worklet processors + any legacy assets
# /assets/* — Vite-built JS/CSS bundles for the React UI
...
@app.get("/")
async def root():
    """Serve the React app's index.html (falls back to legacy static page)."""
...
@app.get("/annotations/{annotation_id}.png")
async def get_annotation(annotation_id: str):
    """Serve an annotated camera frame produced by the annotate_frame tool."""
    data = frame_cache.get_annotation(annotation_id)
    if data is None:
        raise HTTPException(status_code=404, detail="annotation not found")
    # Short cache — the URLs are stable for the session, but content is per-call
