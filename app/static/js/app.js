...
function createImageBubble(imageDataUrl, isUser, caption = null) {
  const messageDiv = document.createElement("div");
  messageDiv.className = `message ${isUser ? "user" : "agent"}`;
...
// Render a shop result from find_nearest_shop
function createShopBubble(topPick, alternates) {
  const messageDiv = document.createElement("div");
  messageDiv.className = "message agent";

  const bubbleDiv = document.createElement("div");
  bubbleDiv.className = "bubble shop-bubble";

  const title = document.createElement("div");
  title.className = "shop-title";
  title.textContent = `🔧 ${topPick.name}`;
  bubbleDiv.appendChild(title);

  const meta = document.createElement("div");
  meta.className = "shop-meta";
  meta.textContent = `${topPick.distance_miles} mi · ${topPick.eta_minutes} min away · ${topPick.hours}`;
  bubbleDiv.appendChild(meta);

  if (Array.isArray(alternates) && alternates.length) {
    const altList = document.createElement("div");
    altList.className = "shop-alternates";
    altList.textContent = "Also nearby: " + alternates.map((s) => s.name).join(", ");
    bubbleDiv.appendChild(altList);
  }
...
// Render a booking confirmation from book_appointment
function createBookingBubble(resp) {
  const messageDiv = document.createElement("div");
  messageDiv.className = "message agent";

  const bubbleDiv = document.createElement("div");
  bubbleDiv.className = "bubble booking-bubble";

  const title = document.createElement("div");
  title.className = "booking-title";
  title.textContent = `✅ Booked — ${resp.shop_name}`;
  bubbleDiv.appendChild(title);

  const meta = document.createElement("div");
  meta.className = "booking-meta";
  meta.textContent = `${resp.time} · ${resp.eta_minutes} min away · #${resp.confirmation_number}`;
  bubbleDiv.appendChild(meta);
...
// Render a manual section snippet from consult_owners_manual
function createManualBubble(title, html) {
  const messageDiv = document.createElement("div");
  messageDiv.className = "message agent";

  const bubbleDiv = document.createElement("div");
  bubbleDiv.className = "bubble manual-bubble";

  const titleDiv = document.createElement("div");
  titleDiv.className = "manual-title";
  titleDiv.textContent = `📘 ${title}`;
  bubbleDiv.appendChild(titleDiv);

  const body = document.createElement("div");
  body.className = "manual-body";
  // Content is bundled by us (or pre-sanitized by the user when dropping files in);
  // we don't accept arbitrary remote HTML here, so direct innerHTML is acceptable.
  body.innerHTML = html || "";
  bubbleDiv.appendChild(body);
  ...
  websocket.onmessage = function (event) {
    ...
    if (adkEvent.content && adkEvent.content.parts) {
      ...
      for (const part of parts) {
        // Handle tool responses (annotate_frame, find_nearest_shop, etc.)
        if (part.functionResponse) {
          const fname = part.functionResponse.name;
          const resp = part.functionResponse.response || {};
          console.log(`[TOOL RESPONSE] ${fname}:`, resp);
          addConsoleEntry("incoming", `Tool: ${fname}`, resp, "🛠️", "tool");

          if (fname === "annotate_frame" && resp.status === "ok" && resp.annotated_image_url) {
            messagesDiv.appendChild(createImageBubble(resp.annotated_image_url, false, resp.instruction || null));
            scrollToBottom();
          } else if (fname === "find_repair_guide" && resp.status === "ok") {
            messagesDiv.appendChild(createManualBubble(resp.title, resp.html));
            scrollToBottom();
          } else if (fname === "find_nearest_shop" && resp.status === "ok" && resp.top_pick) {
            messagesDiv.appendChild(createShopBubble(resp.top_pick, resp.alternates || []));
            scrollToBottom();
          } else if (fname === "book_appointment" && resp.status === "booked") {
            messagesDiv.appendChild(createBookingBubble(resp));
            scrollToBottom();
          }
          continue;
        }
...
/**
 * Camera handling — continuous 1 fps streaming when "Look" mode is on.
 */
...
const stopLookingBtn = document.getElementById("captureImage");

let cameraStream = null;
let lookIntervalId = null;
const LOOK_FPS_MS = 1000; // Live API recommends ~1 fps for video input

async function startLookMode() {
  if (lookIntervalId) return; // already on

  // Prefer the rear camera on phones; gracefully fall back to default if unavailable.
  try {
    ...
    try {
      ...
      addSystemMessage(`Failed to access camera: ${err2.message}`);
      addConsoleEntry("error", "Camera access failed", { error: err2.message, name: err2.name }, "⚠️", "system");
      return;
    }
  }
  ...
  addSystemMessage("👁️ Look mode on — streaming what you see to Otto");
  addConsoleEntry("outgoing", "Look mode on", { fps: 1, mime: "image/jpeg" }, "👁️", "system");

  // Wait for video to have real dimensions before first capture.
  const waitForVideo = () =>
    new Promise((resolve) => {
      if (cameraPreview.videoWidth && cameraPreview.videoHeight) return resolve();
      cameraPreview.addEventListener("loadedmetadata", () => resolve(), { once: true });
    });

  await waitForVideo();
  sendCurrentFrame(); // immediate first frame
  lookIntervalId = setInterval(sendCurrentFrame, LOOK_FPS_MS);
}
...
function stopLookMode() {
  ...
  addSystemMessage("Look mode off");
  addConsoleEntry("outgoing", "Look mode off", null, "🛑", "system");
}
...
// Event listeners — every close path also stops Look mode.
cameraButton.addEventListener("click", startLookMode);
closeCameraModal.addEventListener("click", stopLookMode);
cancelCamera.addEventListener("click", stopLookMode);
stopLookingBtn.addEventListener("click", stopLookMode);
cameraModal.addEventListener("click", (event) => {
  if (event.target === cameraModal) stopLookMode();
});
